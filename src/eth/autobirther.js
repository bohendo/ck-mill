...
const executioner = (lodd, tx) => {
  ...
  var tx
  if (lodd.length === 1) {
    tx = Autobirther.methods.breed1(lodd[0].matronid, lodd[0].blockn).send(tx)

  } else if (lodd.length === 2) {
    tx = Autobirther.methods.breed2(lodd[0].matronid, lodd[0].blockn,
                                    lodd[1].matronid, lodd[1].blockn).send(tx)

  } else if (lodd.length === 3) {
    tx = Autobirther.methods.breed3(lodd[0].matronid, lodd[0].blockn,
                                    lodd[1].matronid, lodd[1].blockn,
                                    lodd[2].matronid, lodd[2].blockn).send(tx)

  } else if (lodd.length === 4) {
    tx = Autobirther.methods.breed4(lodd[0].matronid, lodd[0].blockn,
                                    lodd[1].matronid, lodd[1].blockn,
                                    lodd[2].matronid, lodd[2].blockn,
                                    lodd[3].matronid, lodd[3].blockn).send(tx)

  } else if (lodd.length === 5) {
    tx = Autobirther.methods.breed5(lodd[0].matronid, lodd[0].blockn,
                                    lodd[1].matronid, lodd[1].blockn,
                                    lodd[2].matronid, lodd[2].blockn,
                                    lodd[3].matronid, lodd[3].blockn,
                                    lodd[4].matronid, lodd[4].blockn).send(tx)
  } else {
    tx = Autobirther.methods.breedMore(lodd.map(dd=>dd.matronid)).send(tx)
  }
